...
public class Bird extends JFrame implements KeyListener {
	...
	static int yplayer = 250, score = 0, background = 600,
			xblocks[] = new int[100], blocks[] = new int[100],
			blocks2[] = new int[100];
	static boolean right = false, restart = false, left = false, up = false,
			down = false, block[] = new boolean[100], loser = false;
	...
	public static void main(String[] args) throws InterruptedException {
		...
		blocks2[0] = blocks[0] + 25;
		xblocks[0] = 480;
		block[0] = true;
		for (int i = 1; i < 100; i++) {
			block[i] = false;
			blocks[i] = rand.nextInt(200) + blocks[i - 1] - 40;
			blocks2[i] = blocks[i] + 25;
			while (blocks2[i] > 490) {
				blocks[i] = rand.nextInt(200) + blocks[i - 1] - 40;
				blocks2[i] = blocks[i] + 25;
			}
