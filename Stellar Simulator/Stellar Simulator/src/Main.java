...
public class Main extends JFrame
	...
	public class graphic extends JPanel {
		...
		public void makeAxis(Graphics p) {
			...
			for (int j = 0; j < 15; j++) {
				for (int i = 0; i < 91; i++) {
					...
					circle[(2 * j) + 30][v][i] = (int) ((v==0?500:(v==1?300:0))
							+ getXYZ(20 * Math.sin(((4*i) / (180 / Math.PI))) * Math.sin(/*(angleH-12*j*Math.PI/180<0?Math.PI:0)+*/12 * j / (180 / Math.PI)),
									20 * Math.sin(((4*i) / (180 / Math.PI))) * Math.cos(12 * j / (180 / Math.PI)),
			...
			//	if(circle[i][2][23] > 0 || i<30)
			//	if(i>=30||12*i<angleV*180/Math.PI-90)
			//	p.drawPolyline(circle[i][0], circle[i][1], 91);
				for(int j=0;j<90;j++)
				if(circle[i][2][j]>0||circle[i][2][j+1]>0)
					p.drawLine(circle[i][0][j], circle[i][1][j], circle[i][0][j+1], circle[i][1][j+1]);
			}
			p.drawOval(300, 100, 400, 400);
		}
