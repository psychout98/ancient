...
public class walk extends JFrame implements KeyListener {
	...
	walk() throws InterruptedException {
		super("Walk");
		panel = new paint();
	...
	public double getXYZ(double x, double y, double z, int XYZ) {
		...
		double sA = x1 + .0001 < 0 ? Math.asin(y1 / r) : Math.PI - Math.asin(y1 / r);
		...
		double x2 = dF * -10 * r * Math.cos(((angle - 90) / (180 / Math.PI)) + (sA));
		if (XYZ == 1)
			return x2;
		else if (XYZ == 2)
			return y3;
