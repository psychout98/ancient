...
public class game extends JFrame implements KeyListener {
	...
	game(int boundsRadius) throws InterruptedException {
		super("Walk");
		panel = new paint();
		...
		for(double i=0;i<360;i+=.5){
			OBJ[0].addLine(boundsRadius*Math.cos(i/(180/Math.PI)), boundsRadius*Math.sin(i/(180/Math.PI)), 0, boundsRadius*Math.cos((i+1)/(180/Math.PI)), boundsRadius*Math.sin((i+1)/(180/Math.PI)), 0, Color.red);
		}
		for (int j = 0; j < 10; j++)
			for (int i = 1; i < 11; i++) {
		...
		public void paintComponent(Graphics p) {
			if (started) {
				for (int i = 0; i < OBJ.length; i++) {
					...
					if (y > 0 && x > -400 && x < (2 * center) + 400 || i==0)
						OBJ[i].drawObject(p);
				}
			}
		}
