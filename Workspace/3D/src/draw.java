...
public class draw extends JFrame implements KeyListener, ActionListener,
	...
	int angleH = 315, angleV = 135, editP, editL, editPL, curX, curY, iX, iY,
	...
	draw() throws InterruptedException, FileNotFoundException {
		...
		this.add(flip);
		flip.setSize(80, 40);
		flip.setLocation(this.getWidth() - 100, 40);
		this.setJMenuBar(mb);
		Thread.sleep(500);
	...
	public class cube extends JPanel {
		...
		public void drawObject(int i, int j, Graphics p) {
			...
			case 3:
				...
				p.setColor(PLC[i].getColor().brighter());
				p.fillPolygon(x, y, 4);
				p.setColor(PLC[i].getColor().darker());
				p.drawPolygon(x, y, 4);
				break;
			}
		}
		...
		public void paint(Graphics p) {
			...
			for(int i=0;i<obj[1].length;i++){
				int objP[][][] = new int[obj[0].length][4][6];
				for(int a=0;a<3;a++){
				...
				p.fillPolygon(objP[i][2], objP[i][3], 15);
				p.setColor(Color.black);
			...
			for (int i = 0; i < object.length; i++)
			  ...
			  int[] xh = {100,300,300,100},yh={100,100,200,200}; int[] x2h = {150,250,250,150},y2h={150,150,150,150};
			  int[] nx, ny;
			 
			/*
			CIRCLE C1 = new CIRCLE(PLC[2],-10,-10,5);
			int[] xc = new int[360],yc = new int[360];
			for(int i=0;i<360;i++){
				xc[i] = (int)(center + (zoom * (C1.getX(i))));
				yc[i] = (int)(center2 + (zoom * (C1.getY(i))));
			}
	//		p.setColor(C1.P.c);
			p.setColor(Color.white);
			p.fillPolygon(xc,yc,360);
			*/
			if (highlightPoint) {
			...
			if (Axis) {
				...
				p.drawLine((int) (center + getXY(A[0], 0, 0, true)),
						(int) (center2 + getXY(A[0], 0, 0, false)),
						(int) (center + getXY(A[0] - 2, 0, -2, true)),
						(int) (center2 + getXY(A[0] - 2, 0, -2, false)));
				p.drawLine((int) (center + getXY(-A[0], 0, 0, true)),
						(int) (center2 + getXY(-A[0], 0, 0, false)),
						(int) (center + getXY(-A[0] + 2, 0, 2, true)),
						(int) (center2 + getXY(-A[0] + 2, 0, 2, false)));
				...
				p.drawLine((int) (center + getXY(-A[0], 0, 0, true)),
						...
						(int) (center + getXY(-A[0] + 2, 0, -2, true)),
						(int) (center2 + getXY(-A[0] + 2, 0, -2, false)));
				p.drawLine((int) (center + getXY(0, B[1], 0, true)),
						(int) (center2 + getXY(0, B[1], 0, false)),
						(int) (center + getXY(-2, B[1] - 2, 0, true)),
						(int) (center2 + getXY(-2, B[1] - 2, 0, false)));
				p.drawLine((int) (center + getXY(0, -B[1], 0, true)),
						(int) (center2 + getXY(0, -B[1], 0, false)),
						(int) (center + getXY(2, -B[1] + 2, 0, true)),
						(int) (center2 + getXY(2, -B[1] + 2, 0, false)));
				...
				p.drawLine((int) (center + getXY(0, -B[1], 0, true)),
						...
						(int) (center + getXY(-2, -B[1] + 2, 0, true)),
						(int) (center2 + getXY(-2, -B[1] + 2, 0, false)));
				p.drawLine((int) (center + getXY(0, 0, C[2], true)),
						(int) (center2 + getXY(0, 0, C[2], false)),
						(int) (center + getXY(2, 0, C[2] - 2, true)),
						(int) (center2 + getXY(2, 0, C[2] - 2, false)));
				p.drawLine((int) (center + getXY(0, 0, C[2], true)),
						(int) (center2 + getXY(0, 0, C[2], false)),
						(int) (center + getXY(-2, 0, C[2] - 2, true)),
						(int) (center2 + getXY(-2, 0, C[2] - 2, false)));
				...
				p.drawString(
						...
						(int) (center + getXY(0, (this.getHeight() / 20) - 7.5,
								0, true)),
						(int) (center2 + getXY(0,
								(this.getHeight() / 20) - 7.5, 0, false)));
				p.drawString(
						"Z",
						(int) (center + getXY(0, 0,
								(this.getHeight() / 20) - 7.5, true)),
						(int) (center2 + getXY(0, 0,
								(this.getHeight() / 20) - 7.5, false)));
			}
	...
	public int getXY(double x, double y, double z, boolean XY) {
		...
		double radH = angleH / (180 / Math.PI), radV = angleV / (180 / Math.PI), cP = x
				* Math.sin(radH + (Math.PI / 2)), r = Math.sqrt((y * y)
				+ (z * z)), sA = q == 1 || q == 4 ? Math.asin(z / r) : Math.PI
				- Math.asin(z / r), sH = radH;
		int X = (int) (cP + (r * Math.sin(sH) * Math.cos(radV + sA))), Y = (int) (r * Math
				.sin(radV + sA));
		return XY ?(!flipped ? X : Y ):(!flipped?Y: X);
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource().equals(flip) && flipped){
			flipped = false;
		}
		else if(arg0.getSource().equals(flip) && !flipped){
			flipped = true;
		}
		if (arg0.getSource() == showManager && managing) {
			managing = false;
	...
	public class LINE {
		...
		public int getXs1() {
			return getXY(X1, Y1, Z1, true);
		}

		public int getYs1() {
			return getXY(X1, Y1, Z1, false);
		}

		public int getXs2() {
			return getXY(X2, Y2, Z2, true);
		}

		public int getYs2() {
			return getXY(X2, Y2, Z2, false);
		}

		public double getDepth1() {
			return getDepth(X1, Y1, Z1);
		}

		public double getDepth2() {
			return getDepth(X2, Y2, Z2);
		}
	...
	public class CIRCLE {
		...
		double[] x = new double[360], y = new double[360];
		PLANE P;

		public CIRCLE(PLANE p, double centerX, double centerY, double radius) {
			P = p;
			for (int i = 0; i < 360; i++) {
				x[i] = centerX + (radius * Math.cos(i / (180 / Math.PI)));
				y[i] = centerY + (radius * Math.sin(i / (180 / Math.PI)));
			}
		}

		public double getX(int i) {
			return getXY(P.X[0] + x[i], P.Y[0] + y[i], P.Z[0], true);
		}

		public double getY(int i) {
			return getXY(P.X[0] + x[i], P.Y[0] + y[i], P.Z[0], false);
		}
	}
	...
	public class PLANE {
		...
		public int getX(int i) {
			return getXY(X[i], Y[i], Z[i], true);
		}

		public int getY(int i) {
			return getXY(X[i], Y[i], Z[i], false);
		}

		public int getAverageY() {
			return (getXY(X[0], Y[0], Z[0], false)
					+ getXY(X[1], Y[1], Z[1], false)
					+ getXY(X[2], Y[2], Z[2], false) + getXY(X[3], Y[3], Z[3],
					false)) / 4;
		}

		public int getAverageX() {
			return (getXY(X[0], Y[0], Z[0], true)
					+ getXY(X[1], Y[1], Z[1], true)
					+ getXY(X[2], Y[2], Z[2], true) + getXY(X[3], Y[3], Z[3],
					true)) / 4;
		}

		public double getAveragePlaneDepth() {
			return (getDepth(X[0], Y[0], Z[0]) + getDepth(X[1], Y[1], Z[1])
					+ getDepth(X[2], Y[2], Z[2]) + getDepth(X[3], Y[3], Z[3])) / 4;
		}

		public Color getColor() {
			int r = c.getRed()
					- (((getAverageY() / 3) + (getAverageX() / 3) + (int) (getAveragePlaneDepth() / 3))), g = c
					.getGreen()
					- (((getAverageY() / 3) + (getAverageX() / 3) + (int) (getAveragePlaneDepth() / 3))), b = c
					.getBlue()
					- (((getAverageY() / 3) + (getAverageX() / 3) + (int) (getAveragePlaneDepth() / 3)));
			r = r >= 255 ? 255 : r <= 0 ? 0 : r;
			g = g >= 255 ? 255 : g <= 0 ? 0 : g;
			b = b >= 255 ? 255 : b <= 0 ? 0 : b;
	...
	public class POINT {
		...
		public int getXs() {
			return getXY(X, Y, Z, true);
		}

		public int getYs() {
			return getXY(X, Y, Z, false);
		}

		public double getPointDepth() {
			return getDepth(X, Y, Z);
		}
	}
