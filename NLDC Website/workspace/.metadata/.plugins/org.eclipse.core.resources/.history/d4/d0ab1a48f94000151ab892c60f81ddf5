...
public class site extends JFrame implements ActionListener {
	...
	JPanel panel = new JPanel(), panel3 = new JPanel(new BorderLayout()),
			...
			panel6 = new JPanel(), panel2 = new JPanel(), tpan = new JPanel(
					new BorderLayout()), p8 = new JPanel(), p7[];
			...
			if (CFS[i].Location == loc) {
				if (CFS[i].Type == 0) {
					if (!p.equals(mfp[0])) {
						...
						JPanel tppp = new JPanel(new BorderLayout()),tpp2 = new JPanel(new GridLayout(CFS[i].rb.length,1));
						for (int j = 0; j < CFS[i].rb.length; j++)
							tpp2.add(CFS[i].rb[j]);
						tppp.add(tpp2);
						p.add(tppp);
	...
	public class customField extends JFrame implements ActionListener,
		...
		public void addCF() {
			int loca;
			/*
			if (cb.getSelectedIndex() > standards.length - 1) {
				if (CFS[cb.getSelectedIndex() - standards.length].Type == 3)
					loca = cb.getSelectedIndex();
				else
					loca = CFS[cb.getSelectedIndex() - standards.length].Location;
			} else */
				loca = cb.getSelectedIndex();
			boolean valid = true;
			for (int i = 0; i < CFS.length; i++) {
