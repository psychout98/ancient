...
public class site3 extends JFrame implements ActionListener {
	...
	public site3(String name) throws IOException, InterruptedException {
	...
	public JPanel showHTML(){
		...
		JEditorPane ed1=new JEditorPane("text/html",writer());
	...
	public String writer(){
		...
		form += (sel[15] ? "<div style=\"width:760px;border:1px solid gray;background:#6AA840;\">DRIVE / RESOURCE ACCESS<br>"
				: "");
		form += (sel[16] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Which networked drives should the user have access to?<br><br>"
				+ acc(16) + "</div>"
				: "");
		form += (sel[17] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Which calendars should the user have access to?<br><br>"
				+ acc(17) + "</div>"
				: "");
		form += (sel[18] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Should another employee have access to the new user's calendar?<br><br>"
				+ acc(18) + "</div>"
				: "");
		form += (sel[19] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Should the user have access to another user's email?<br><br>"
				+ acc(19) + "</div>"
				: "");
		form += (sel[20] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Should another employee have access to the new user's email?<br><br>"
				+ acc(20) + "</div>"
				: "");
		form += (sel[21] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Should the user be included in any of the following email distribution lists?<br><br>"
				+ acc(21) + "</div>"
				: "");
		form += (sel[22] ? "<div style=\"border:1px solid gray;width=760px;\"><br>Which shared peripherals should the user have access to?<br><br>"
				+ acc(22) + "</div>"
				: "");
		form += pcc(2);
		form += acc(15);
		form += (sel[15] ? "</div><br><br>" + pcc(3) : "");
		form += (sel[23] ? "<div style=\"width:760px;border:1px solid gray;background:#6AA840;\">APPLICATION ACCESS<br>"
		...
		String f = "<Html><Head><Title>"
				...
				+ "<div style=\"margin:10px\"><font  size=\"4\" color=\"black\" face=\"Ariel\">\""
	...
	public void acf(int loc, JPanel p) {
		for (int i = 0; i < CFS.length; i++) {
			if (CFS[i].Location == loc) {
				if (CFS[i].Type == 0) {
					p.setLayout(new GridLayout(Integer
							.parseInt(p
									.getLayout()
									.toString()
									.substring(
				...
				if (CFS[i].Type == 0) {
					for (int j = 0; j < CFS[i].rb.length; j++)
						...
						fc += "<input type=\"radio\"name=\""
								+ CFS[i].rb[0].getText()
								+ "\"\"value=\"\"+userinfo[0]+\"\">"
								+ CFS[i].rb[j].getText() + "<br>";
				}
				if (CFS[i].Type == 1) {
					fc += "<input type=\"checkbox\" value=\"\"+userinfo[0]+\"\">"
							+ CFS[i].Label.getText()+"<br>";
				}
				if (CFS[i].Type == 2) {
					fc += CFS[i].Label.getText()
							+ " <input type=\"text\" value=\"\"+userinfo[0]+\"\"><br>";
				}
				if (CFS[i].Type == 4) {
					fc += CFS[i].Label.getText()+"<br>";
				}
			}
		}
		return fc;
	...
	public class editFields extends JFrame implements ActionListener,
		...
		JLabel l[] = { new JLabel("Choose field: "),
				new JLabel("Change Label: "),
				new JLabel("Change Location"), new JLabel("Radiobutton:") };
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == bts[1]) {
				if (CFS[cb.getSelectedIndex()].Type == 0) {
					CFS[cb.getSelectedIndex()].rb[num].setText(tp.getText());
		...
		public void init2(){
			...
			if (CFS[cb.getSelectedIndex()].Type == 0) {
				...
				cb4 = new JComboBox(rbs);
			}
			else if (CFS[cb.getSelectedIndex()].Type == 3) {
				cb3 = new JComboBox(sections);
				cb4 = new JComboBox();
			}
			else {
				cb3 = new JComboBox(locats);
					...
					if (Type == 0) {
						if (cr.getSelectedIndex() > 0) {
							...
							for (int i = 0; i < tf.length; i++)
								CFS[i] = tf[i];
							CFS[CFS.length - 1] = new field(
									cb.getSelectedIndex(), Type, t.getText());
							this.dispose();
							Panel2();
						}
					} else {
